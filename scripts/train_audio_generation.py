"""Train or exactly resume the text-conditioned latent audio diffusion model."""
...
from audio_generation.dataset import AudioCaptionDataset
...
def _checkpoint_metadata(config, tokenizer):
    return {
        "task": "text_to_audio",
        "config": config,
...
def main() -> None:
    ...
    validate_generation_config(config, kind="audio")
    ...
    dataset = AudioCaptionDataset(
        ...
        crop=str(config.get("train_crop", "random")),
        peak_normalize=bool(config.get("peak_normalize", True)),
    )
    ...
    if config.get("validation_manifest"):
        validation_dataset = AudioCaptionDataset(
            config["validation_manifest"],
            sample_rate=int(config["sample_rate"]),
            duration_seconds=float(config["duration_seconds"]),
            crop=str(config.get("validation_crop", "center")),
            peak_normalize=bool(config.get("peak_normalize", True)),
        )
    ...
    steps_per_epoch = optimizer_steps_per_epoch(len(loader), accumulation)
    ...
    output = Path(config.get("output", "checkpoints/audio_generation/latest.pt"))
    best_output = Path(config.get("best_output", "checkpoints/audio_generation/best.pt"))
    metadata = _checkpoint_metadata(config, tokenizer)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        model.train()
        group_start = 0
        for batch_index, (waveforms, texts) in enumerate(loader):
            ...
            with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=amp_enabled):
                ...
                scaled_loss = loss / current_group
            if not torch.isfinite(loss):
                raise FloatingPointError(f"non-finite audio generation loss at step {step}")
            ...
            if not torch.isfinite(torch.as_tensor(grad_norm)):
                raise FloatingPointError(f"non-finite audio gradient norm at step {step}")
            ...
            if step % int(config.get("log_every_steps", 20)) == 0:
                print(
            ...
            if step % int(config.get("save_every_steps", 500)) == 0:
                save_checkpoint(
                    output,
                    model,
                    optimizer=optimizer,
        ...
        if validation_loader is not None:
            ...
            with ema.average_parameters(model), torch.inference_mode():
                for waveforms, texts in validation_loader:
                    waveforms = waveforms.to(device, non_blocking=True)
                    features = _features(model, tokenizer, texts, device)
