...
def collect_records(
    ...
    selected = sorted(candidates.values(), key=lambda item: item[0])[:limit]
    return [item for _, item in selected], stats


def build_dataset(data_root: Path, output: Path) -> dict[str, Any]:
...
def main() -> None:
    ...
    print(json.dumps(build_dataset(args.data_root, args.output), indent=2))
