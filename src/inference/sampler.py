...
def validate_constraint_mask(logits: Tensor, allowed: Tensor) -> Tensor:
    """Apply a token-time allow mask and fail closed if it blocks every token."""
    if allowed.dtype is not torch.bool or allowed.shape != logits.shape:
        raise ValueError("allowed mask must be boolean and match logits")
    filtered=logits.masked_fill(~allowed, float("-inf"))
    if not torch.isfinite(filtered).any(dim=-1).all():
        raise ValueError("structured-output constraint blocked every token")
    return filtered
